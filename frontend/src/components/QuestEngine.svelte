<script>
    ...
    function triggerSticker(type) {
        ...
        const randomRot = baseRot + (Math.random() - 0.5) * 10; // +/- 3deg variance
    ...
    function trackStat(nodeId, type) {
        if (!sessionStats[nodeId])
            sessionStats[nodeId] = { correct: 0, wrong: 0, hint: 0 };
    ...
    async function handleAnswer(userAnswer) {
        ...
        if (isCorrect) {
            ...
            if ($user && currentQuestion.node_id) {
                const masteryStatus =
                    attempts === 0 && !wasHinted ? "MASTERED" : "LEARNED";
            ...
            wrongAttempts[currentQuestion.id] =
                (wrongAttempts[currentQuestion.id] || 0) + 1;
    ...
    async function requestAIFeedback(userAnswer) {
        ...
        try {
            const res = await fetch(
                "http://localhost:8000/api/v1/quest/ai-correction",
                {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({
            ...
            isAiThinking = false;
            aiFeedbackText = buildFallbackFeedback(
                currentQuestion,
                lastUserAnswer,
                correctAnswerDisplay,
            );
...
<div
    class="quest-engine h-full flex flex-col items-center justify-center p-6 text-slate-200"
>
    <div
        class="w-full max-w-2xl glass-panel rounded-[2.5rem] p-10 shadow-2xl relative overflow-hidden flex flex-col min-h-[520px] max-h-full"
    >
        <!-- Subtle Glow -->
        <div
            class="absolute -top-20 -right-20 w-64 h-64 bg-indigo-200/20 rounded-full blur-[80px] pointer-events-none"
        ></div>
        ...
        <div class="flex items-center gap-6 mb-10 relative z-10 shrink-0">
            <button
                on:click={onQuit}
                class="w-10 h-10 rounded-xl bg-white/20 hover:bg-rose-500/10 hover:text-rose-600 transition flex items-center justify-center text-slate-400"
            >
                <svg
            ...
            <div
                class="flex-grow bg-slate-200/50 h-2.5 rounded-full overflow-hidden border border-white/40"
            >
                <div
            ...
            <div
                class="px-4 py-2 bg-indigo-500 text-white font-black rounded-xl shadow-lg shadow-indigo-500/20 flex items-center gap-2"
            >
        ...
        <div
            class="flex-grow flex flex-col overflow-y-auto custom-scroll pr-1 relative z-10"
        >
            {#if currentQuestion}
                    ...
                    {#key currentQuestion.id + currentIndex}
                        ...
                        >
                            ...
                            <div class="text-center mb-6">
                                ...
                                >
                                    <span
                                        class="inline-block px-3 py-1 bg-indigo-100 text-indigo-600 rounded-lg text-[10px] font-black uppercase tracking-[0.2em]"
                                    >
                                ...
                                {#if currentQuestion.grammar_focus}
                                    <p
                                        class="text-xs text-indigo-400 font-semibold mb-3 opacity-80"
                                    >
                                        📚 {currentQuestion.grammar_focus}
                                    </p>
                                {/if}
                                <h3
                                    class="text-2xl md:text-[1.65rem] font-black text-white leading-tight"
                                >
                                    {@html applyFurigana(
                                    ...
                                    <div class="flex flex-col items-center">
                                        <input
                                            ...
                                            class="w-full p-5 text-center rounded-2xl bg-white/90 border-2 transition-all text-xl font-bold text-slate-900 placeholder:text-slate-400 shadow-inner
                                                ...
                                                : 'border-white/60 focus:border-indigo-400 focus:bg-white'}
                                        ...
                                        {#if lastAnswerWrong}
                                            <p
                                                class="text-rose-500 text-sm font-semibold mt-2"
                                                in:fade
                                            >
                            ...
                            {:else if currentQuestion.type === "translate"}
                                {#if !showAiFeedback}
                                    <div class="flex flex-col items-center">
                                        <div
                                            class="w-full mb-3 p-4 bg-amber-500/10 border border-amber-500/25 rounded-2xl text-sm text-amber-300 font-medium text-center"
                                        >
                                            💡 Tulis dalam romaji atau hiragana.
                                            Tidak perlu huruf kapital atau tanda
                                            baca.
                                        </div>
                                        <textarea
                                            bind:value={fillAnswer}
                                            ...
                                            class="w-full p-5 text-center rounded-2xl bg-white/90 border-2 transition-all text-lg font-bold text-slate-900 placeholder:text-slate-400 shadow-inner resize-none
                                                ...
                                                : 'border-white/60 focus:border-amber-400 focus:bg-white'}
                            ...
                            {#if !showAiFeedback && currentQuestion.hint}
                                <div class="mt-6 text-center">
                                    {#if !isHintVisible}
                                        ...
                                        >
                                            🔍 Lihat Petunjuk <span
                                                class="opacity-60">(−2 XP)</span
                                            >
                                        </button>
                                    {:else}
                                        <div
                                            class="inline-block px-4 py-3 bg-slate-800 border border-slate-700 rounded-xl text-sm text-slate-200 font-medium"
                                            in:fly={{ y: 5 }}
                                        >
                                            💡 {@html applyFurigana(
            ...
            {#if showAiFeedback}
                ...
                >
                    ...
                    <div class="flex flex-col sm:flex-row gap-4 w-full">
                        <div
                            class="flex-1 p-4 bg-rose-500/10 border border-rose-500/20 rounded-2xl flex flex-col shadow-sm backdrop-blur-sm"
                        >
                            <span
                                class="text-[10px] text-rose-400 font-bold uppercase tracking-wider mb-1"
                                >Jawaban Kamu</span
                            >
                            <span
                                class="text-sm font-bold text-rose-200 line-through leading-relaxed"
                                >{lastUserAnswer || "-"}</span
                            >
                        </div>
                        <div
                            class="flex-1 p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-2xl flex flex-col shadow-sm backdrop-blur-sm"
                        >
                            <span
                                class="text-[10px] text-emerald-400 font-bold uppercase tracking-wider mb-1"
                                >Jawaban Benar</span
                            >
                            <span
                                class="text-sm font-bold text-emerald-200 leading-relaxed"
                                >{@html applyFurigana(correctAnswerDisplay) ||
                                    "-"}</span
                            >
                        </div>
                    </div>
                    ...
                    <!-- Explanatory Bubble -->
                    <div
                        class="p-8 glass-card border-indigo-200/50 bg-indigo-50/30 relative overflow-hidden"
                    >
                        <div
                            class="absolute -top-10 -right-10 w-32 h-32 bg-indigo-500/5 rounded-full blur-2xl"
                        ></div>
                        ...
                        {#if isAiThinking}
                            <div class="thinking-content py-2">
                                ...
                                <p class="thinking-label">
                                    A.L.I.S.A. sedang merangkai penjelasan...
                                </p>
                            </div>
                        {:else}
                            <p
                                class="text-slate-200 font-semibold leading-relaxed mb-6 italic"
                            >
                                "{@html applyFurigana(aiFeedbackText)}"
                            </p>
